...
function getDianzan(p){
    ...
    $.ajax({
        //type:"get",
        url:"ajaxGetDianzan/p/"+p,//调用ajax获取首页点赞记录接口
        //dataType:"json",
        success:function(data)
        {
            ...
            var html='<table border="1px" cellspacing="0px" width="100%" style="border: solid 1px;" cellpadding="0" cellspacing="0">' +
                '<tr style="width:30px;"><th>用户名</th><th>点赞结果</th><th>点赞时间</th>';
            ...
            $(data.info.data).each(function(k,v){
                ...
                html +='<tr class="tron"><th>'+v.username+'</th><th>' +v.dianzan+'</th><th>' +v.dianzantime+'</th></tr>';
            });

            html=html+'</table>';//最后才加上闭合标签</table>

            $("#dianzanjilu").replaceWith("<div id='dianzanjilu'>"+html+"</div>");
            ...
            document.getElementById('dianzanjilu_page').innerHTML='<br /><font size="1">当前页/总页数</font>&nbsp;<font color="red" size="4">'+
                p+'</font>/<font color="red" size="4">'+data.info.pageNumber+'</font><br /><br /><input type="button" value="<<" onclick="getDianzan(1)" />' +
...
//ajax获取站长公告
function getZhanZhangGongGao(){

    $.ajax({
        url:"getZhanZhangGongGao",
        success:function(data)
        {
            alert(data);
        }
...
function dianzan(){
    ...
    //先把按钮禁用
    $('#dzdz').attr('disabled',"true");
    $.ajax({
        url:"dianzan",
        success:function(data)
        {
            //ajax_get_leijihuodeqb_dianzancishu_totledianzancishu();
            ...
            //恢复按钮禁用
            $('#dzdz').removeAttr("disabled");
        }
    });

}

//ajax获取累计获得多少QB（点赞是获得QB的唯一渠道）  当前用户点赞次数  网站总共被点赞次数
function ajax_get_leijihuodeqb_dianzancishu_totledianzancishu(){

    var xhr0 = new XMLHttpRequest();
    xhr0.onreadystatechange = function(){
        if(xhr0.readyState==4){
            document.getElementById('dz0').innerHTML=xhr0.responseText;
        }
        else{
            document.getElementById('dz0').innerHTML='<image src="/Statics/image/wait.jpg" />';
        }
    }
    xhr0.open('get','ajax_get_leijihuodeqb');
    xhr0.send(null);

    var xhr1 = new XMLHttpRequest();
    xhr1.onreadystatechange = function(){
        if(xhr1.readyState==4){
            document.getElementById('dz1').innerHTML=xhr1.responseText;
        }
        else{
            document.getElementById('dz1').innerHTML='<image src="/Statics/image/wait.jpg" />';
        }
    }
    xhr1.open('get','ajax_get_dianzancishu');
    xhr1.send(null);

    var xhr2 = new XMLHttpRequest();
    xhr2.onreadystatechange = function(){
        if(xhr2.readyState==4){
            document.getElementById('dz2').innerHTML=xhr2.responseText;
        }
        else{
            document.getElementById('dz2').innerHTML='<image src="/Statics/image/wait.jpg" />';
        }
    }
    xhr2.open('get','ajax_get_totledianzancishu');
    xhr2.send(null);
...
function ajax_get_myqb_mycfdian(){
    ...
    xhr1.onreadystatechange = function(){
        if(xhr1.readyState==4){
            ...
            document.getElementById('myqb').innerHTML='<image src="/Statics/image/wait.jpg" />';
        }
    }
    xhr1.open('get','ajax_get_myqb');
    xhr1.send(null);
    ...
    xhr2.onreadystatechange = function(){
        if(xhr2.readyState==4){
            ...
            document.getElementById('mycfdian').innerHTML='<image src="/Statics/image/wait.jpg" />';
        }
    }
    xhr2.open('get','ajax_get_mycfdian');
    xhr2.send(null);
...
/*function timer(time){
 ...
 btn.val(time <= 0 ? "点我点赞1" : ("" + (time) + "秒后可再次点赞"));
 ...
 btn.val("" + (time--) + "秒后可再次点赞");
 }
 }, 1000);

 $.ajax({
 url:"http://cf/index.php/Home/Login/dianzan",
 success:function(data)
 {
 alert(data);
 }
